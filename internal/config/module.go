package config

import (
	"fmt"
	"os"

	"github.com/android-sms-gateway/ca/internal/api"
	"github.com/android-sms-gateway/ca/internal/csr"
	"github.com/android-sms-gateway/core/http"
	"github.com/android-sms-gateway/core/redis"
	"go.uber.org/fx"
)

func Module() fx.Option {
	return fx.Module(
		"config",
		fx.Provide(New, fx.Private),
		fx.Provide(func(c Config) http.Config {
			return http.Config{
				Address:     c.HTTP.Address,
				ProxyHeader: c.HTTP.ProxyHeader,
				Proxies:     c.HTTP.Proxies,
			}
		}),
		fx.Provide(func(c Config) redis.Config {
			return redis.Config{
				URL: c.Storage.URL,
			}
		}),
		fx.Provide(func(c Config) api.Config {
			return api.Config{
				CORSAllowOrigins: c.API.CORSAllowOrigins,
			}
		}),
		fx.Provide(func(c Config) (csr.Config, error) {
			caCert, err := os.ReadFile(c.CSR.CACertPath)
			if err != nil {
				return csr.Config{}, fmt.Errorf("failed to read ca cert: %w", err)
			}

			caKey, err := os.ReadFile(c.CSR.CAKeyPath)
			if err != nil {
				return csr.Config{}, fmt.Errorf("failed to read ca key: %w", err)
			}

			return csr.Config{
				CACert: caCert,
				CAKey:  caKey,
				TTL:    c.CSR.TTL,
			}, nil
		}),
	)
}
